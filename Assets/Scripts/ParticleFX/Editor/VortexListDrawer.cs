...
[CustomPropertyDrawer(typeof(VortexLap.VortexList))]
public class VortexListDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        {
            var circuit = property.FindPropertyRelative("_lap").objectReferenceValue as VortexLap;
            ...
            circuit._vortexList._items = new Transform[children.Length];
            for (n = 0; n < children.Length; ++n)
            {
                circuit._vortexList._items[n] = children[n];
            }
        }
        ...
        if (GUI.Button(renameButtonRect, "Auto Rename numerically from this order"))
        {
            var circuit = property.FindPropertyRelative("_lap").objectReferenceValue as VortexLap;
            int n = 0;
            foreach (var child in circuit._vortexList._items)
            {
                child.name = "VortexPoint " + (n++).ToString("000");
            }
        }
